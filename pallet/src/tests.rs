...
use crate::helper::balance_of;
use crate::{LpBalances, mock};
use crate::mock::*;
use crate::{Error, Event, reserves, Reserves};
...
fn withdraw_works() {
    new_test_ext().execute_with(|| {
        ...
        Amm::deposit_asset_1(Origin::signed(origin_to),deposit_amount).expect("deposit should work");
...
fn deposit_and_withdraw_work() {
    new_test_ext().execute_with(|| {
        ...
        // do second deposit
        Amm::deposit_asset_1(Origin::signed(origin_to), deposit_amount).expect("2nd deposit should work");
        ...
        match &burn_event.event {
            mock::Event::Amm(Event::Burn { sender: _, to: _, amount_0, amount_1 }) => {
                assert_eq!(amount_0, &deposit_amount, "expected withdrawn amount_0 to be == to deposited amount");
                assert_eq!(amount_1, &deposit_amount, "expected withdrawn amount_1 to be == to deposited amount");
            }
            _ => {
                assert!(false);
            }
